...
###Download variables. It is necessary to have an account in ECMWF. Then, download the package ecmwfr, and run the following function: 
...
for (i in variable_list) {
  ...
  grib_file <- paste0(dir, "descarregues/", i,".grib") #Open the grib and read as rasterbrick
...
evapotranspiration<-raster::subset(evaporation_from_vegetation_transpiration, c(4:7, 16:19, 28:31, 52:55)) #select apr-jul from year 1 to 5
evapotranspiration<-calc(evapotranspiration, fun = mean, na.rm=T)

##Mean annual temperature
MAT<-calc(`2m_temperature`, fun=mean, na.rm=T)

##Mean temperature in breeding period (APR-JUL)
MATBP<-raster::subset(`2m_temperature`, c(4:7, 16:19, 28:31, 52:55))
MATBP<-calc(MATBP, fun=mean, na.rm=T)
...
TAP<-calc(total_precipitation, fun=sum, na.rm=T)
TAP<-TAP/5

##Total precipitation in the breeding period
TAPBP<-raster::subset(total_precipitation, c(4:7, 16:19, 28:31, 52:55)) #select apr-jul from year 1 to 5
TAPBP<-calc(TAPBP, fun = sum)
TAPBP<-TAPBP/5
...
days_snow_cover<-raster::reclassify(snow_cover, rcl)

##Surface net solar radiation
solar_radiation<-calc(surface_net_solar_radiation, fun=mean, na.rm=T)
